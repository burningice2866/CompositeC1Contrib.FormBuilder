...
namespace CompositeC1Contrib.FormBuilder.Dynamic.Wizard.C1Console.ElementProvider
{
    ...
    public class FormWizardFolderEntityTokenHandler : IEntityTokenBasedElementProvider
    {
        ...
        public IEnumerable<Element> Handle(ElementProviderContext context, EntityToken token)
        {
            ...
            var wizard = DynamicFormWizardsFacade.GetWizards().SingleOrDefault(w => w.Name == wizardFolderEntityToken.WizardName);
            if (wizard != null)
            {
        ...
        private static IEnumerable<Element> HandleSubmitHandlersFolder(ElementProviderContext context, DynamicFormWizard wizard)
        {
            foreach (var handler in wizard.SubmitHandlers)
            {
                ...
                var editWorkflowAttribute = handler.GetType().GetCustomAttribute<EditWorkflowAttribute>();
